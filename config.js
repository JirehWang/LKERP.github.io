...
(function() {
  ...
  // 🛡️ 防呆檢查：如果抓到 DEFAULT，直接終止設定並拋出警告
  if (currentKey === "DEFAULT" || !targetEncodedUrl) {
    console.error(`🚨 [路由錯誤] 找不到專案 '${repoName}' 或 '${hostname}' 的後端網址設定！請檢查 config.js。`);
    window.GAS_URL = null; 
  } else {
  ...
  window.churchAPI = async function(action, data = {}) {
    // 🛡️ 在發送請求前，再次攔截沒有網址的無效連線
    if (!window.GAS_URL) {
      const errorMsg = "系統尚未就緒：找不到對應的後端網址 (GAS_URL 為空)。";
